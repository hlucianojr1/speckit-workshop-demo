...
// dense-array index order, to preserve determinism (Article 5, FR-009).
...
- **`turbulence_force`**: `apply_force` draws exactly three values from `rng` via
  ...
  `strength`, and writes the result into `out_acceleration`. `frequency` is stored but does
  not change the per-call draw count in v1 (reserved for a future time-varying noise
  function without a breaking signature change).
- **Composition** (FR-007): callers (see `contracts/emitter.md`) sum every attached
  applicator's `out_acceleration` before integrating it into a particle's velocity for that
  tick (`velocity += sum(acceleration) * dt`), per the additive-composition decision in
  research.md §3. One emitter's forces never read or affect another emitter's particles
  (FR-008) because each `force_applicator` list is owned per-emitter.
- **No allocation**: `apply_force` never allocates; `force_applicator` values are stored by
  value in a pre-reserved `eastl::vector` on the owning `emitter` (Article 6).
...
- Edge case: `turbulence_force{ strength: 0.0f }` always yields `{0, 0, 0}` regardless of
  `rng` state, but still consumes exactly 3 draws from `rng` (the consumption count must stay
