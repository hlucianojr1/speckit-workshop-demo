...
> OpenGL driver. Fix it in step 6.
...
## 6. VM fix — software OpenGL (Mesa llvmpipe)
...
| `WGL: The driver does not appear to support OpenGL` + assertion in `window.c` | No hardware OpenGL (VM/RDP). Apply step 6. |
...
## 8. Quick reference — full clean setup, one block
...
.\build\apps\sandbox\ea-sandbox.exe --headless --seed 42 --frames 600 --out trace.csv
.\build\apps\sandbox\ea-sandbox.exe    # interactive (apply step 6 first on a VM)
```
